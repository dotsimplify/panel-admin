  ...
  return (
    ...
    >
      <h2
        ...
        } select-none mb-2  text-gray-700 min-w-max `}
      >
        {props.label}
      </h2>
...
text-gray-700 bg-gray-100 bg-clip-padding bg-no-repeat transform cursor-pointer transition ease-in-out m-0 duration-500 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none`}
        >
          {props.value ? props.value : props.placeholderText}
        </h2>

        {props.touch && (
          <ul className="absolute z-40 max-h-48 overflow-y-auto w-full top-9  border-l border-r border-b  border-gray-300 bg-white shadow-md ">
            <input
              onKeyUp={keyUpSearch}
        ...
        {!props.touch ? (
          <BsChevronDown className="absolute right-4 top-2  " />
        ) : (
          <BsChevronUp className="absolute right-4 top-2  " />
        )}
      </div>
    </section>
  );
